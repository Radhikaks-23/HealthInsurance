...
claims_df = spark.read.csv("/mnt/datalakehousegen23/landing/Claims/*.csv", header=True)

claims_df = claims_df.withColumn(
    "datasource",f.when(f.input_file_name().contains("hospital1"),"hosa").when(f.input_file_name().contains("hospital2"),"hosb")
    .otherwise(None)
)
...
claims_df.write.mode("overwrite").format("Parquet").save("/mnt/datalakehousegen23/bronze/claims")

# COMMAND ----------

claims_df = spark.read.parquet("/mnt/datalakehousegen23/bronze/claims")
display(claims_df)
